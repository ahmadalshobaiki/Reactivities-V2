    ...
    IEmailSender<User> emailSender, IConfiguration config) : BaseApiController
{
    ...
    public async Task<ActionResult> LoginWithGithub(string code)
    {
        ...
        // http client for our API server (similar to axios in our SPA frontend)
        ...
        httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", tokenContent.AccessToken); // include the acces token in the header of our subsequent requests
