...
public class ExceptionMiddleware(ILogger<ExceptionMiddleware> logger, IHostEnvironment env) : IMiddleware
{
    public async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        ...
        catch (ValidationException ex) // exception at the DTO Level
        {
            await HandleValidationException(context, ex);
        }
        catch (Exception ex) // exception at the internal server Level
        {
            await HandleValidation(context, ex);
        }
    }

    private async Task HandleValidation(HttpContext context, Exception ex)
    {
