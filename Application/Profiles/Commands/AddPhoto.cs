...
public class AddPhoto
{
    ...
    IPhotoService photoService) : IRequestHandler<Command, Result<Photo>>
    {
        public async Task<Result<Photo>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            return result
            ? Result<Photo>.Success(photo) // return a Result<T> object where T is a photo object. We encapsulate T in the Result object so we can keep its values while enriching it with Result properties to be able to process it further
